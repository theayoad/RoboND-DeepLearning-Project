...
To establish a viable set of hyperparameters, specifically, values for learning rate, batch size, number of epochs, and steps per epoch I used the segmentation notebook from the [RoboND Segmentation Lab](https://github.com/udacity/RoboND-Segmentation-Lab). Due to slow computation speed of my laptop, I initially fixed the number of epochs at 2, while I optimized the other hyperparameters. It took approximately 1 hour for each epoch on my laptop (which I why I made the transition to a p2.xlarge instance hosted by AWS when validating sets of hyperparameters after I found workable ranges for each hyperparameter). 
